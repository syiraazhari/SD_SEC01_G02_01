...
Public Class frmSales
    ...
    Private Sub btnPayment_Click(sender As Object, e As EventArgs) Handles btnPayment.Click
        ...
        '    DT.Rows.Add(Now.ToString("yyyy-MM-dd"), lblTime.Text, dr.Cells(2).Value, dr.Cells(4).Value, FormatCurrency(CDbl(dr.Cells(5).Value)), FormatCurrency(CDbl(dr.Cells(6).Value)), FormatCurrency(CDbl(lblSubTotal.Text)), FormatCurrency(CDbl(lblTax.Text)), FormatCurrency(CDbl(lblCash.Text)), FormatCurrency(CDbl(lblChange.Text)), FormatCurrency(CDbl(lblNetTotal.Text)))
        'Next
    ...
    Sub LoadStock()
        For i = 0 To (Me.Guna2DataGridView1.Rows.Count - 2)                         ' For i = 0 To (Me.Guna2DataGridView1.Rows.Count - 1)
            ...
            Else
                cn.Open()
                cmd = New MySqlCommand("insert into tbl_stock (PID, Description, Category, PurQty, SalesQty) values(@PID, @Description, @Category, @PurQty, @SalesQty)", cn)
                With cmd
                    .Parameters.AddWithValue("@PID", PID_)
                    .Parameters.AddWithValue("@Description", Desc_)
                    .Parameters.AddWithValue("@Category", Cat_)
